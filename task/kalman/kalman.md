...
### 环境调节研究(2026-09-06): 弱市收紧突破入场 — 证伪归档
**假设**: 池级 up 占比 < 阈值(弱势环境)时提高新建仓偏离要求(2%→3%), 减少弱势追突破损耗(9-1~9-4 巨石/香农/大族止损潮为活体动机)。
**实现**: 预计算器 `backtest.compute_env_up_map`(SignalEngine 逐日全池 trend 统计, warmup 50 与策略一致) → 注入策略 `env_up_map`; `strategy.py` 弱市检查(T-1 日 up 占比 < env_weak_threshold 且偏离 < env_strict_entry → 跳过新建仓, 无前视)。
**验证(修复引擎, stock 池 30 万)**:
- 2021-2022 弱势年: weak<35%/偏离≥3% 调节 **-9.2pp**(32.0%→22.8%); weak<25% 变体 -0.7pp(接近持平, 无正增益)
- 全区间 2020-2026: weak<25%/偏离≥3% **+20.8pp**(225.9%→246.7%), 但年度分解**增益几乎全部来自 2023 单年(+17.1pp)**, 其余 2021 -0.4/2022 -0.1/2024 **-6.0**/2025 **-3.7**/2026 -0.4 全负或持平——与 MA20 止盈"单年依赖=不推荐"同模式(5/7 年负)
**结论**: 池级环境收紧路线证伪——与 8-03"震荡/环境识别无系统增益"再次呼应。代码保留默认关(env_filter_enabled=False, 研究用参数), 不入生产配置。
**下一步候选(2026-09-06 摸底)**: 假突破量能特征——2,125 笔买入中, **量比分桶快速反转率单调递减(Q1 缩量 74% → Q4 放量 51%, -23pp)**; 区间高位追入 76% 反转率(弱辅助)。量能维度是 8-03 证伪范围(价格形态特征)之外的新特征, 可分性成立, 待组合收益关验证(注意 BBANDS 前车之鉴: 单标的有效但组合不占优)。

### 假突破量能过滤研究(2026-09-06): 组合收益关不过 — 证伪归档
**动机**: 摸底 2,125 笔买入显示量比单调可分——缩量买入(量比 Q1)快速反转率 74%, 放量(Q4)51%; 量能是 8-03 证伪范围(价格形态)之外的新维度。
**单笔盈亏验证(1,660 笔配对)**: Q1 缩量平均 **-218 元/笔(合计 -9.0 万, 胜率 24.8%)** = 真实亏损源; Q4 放量 +1,919 元/笔(合计 +79.6 万, 胜率 45.3%)——**方向正确**。
**组合验证(修复引擎, stock 池 30 万, 全区间 2020-2026, strategy.py volume_filter_enabled)**:
- 量比<1.0 过滤: **-111.3pp**(225.9%→114.6%, 2024 年转亏 -5.6%)——阈值误伤 Q2/Q3 正期望单(量比中位 ~1.0, 过滤了近半买入)
- 量比<0.6(≈精确 Q1): **-6.7pp**(219.2%)——消除 -9 万亏损理论上应 +2.5pp, 实际为负: 被过滤单的资金/名额被次优替代 + 信号时点偏移(今日缩量被拒 → 后续再触发价已变)
**结论**: 单笔特征可分(Q1 是亏损源)但**组合收益关不过**——替代效应与信号时点偏移系统性吞掉过滤收益。与 BBANDS/8-03 同模式: "单笔特征过滤"在组合层难转化(唯一例外 MFI——池级资金行为 +57pp, 非价格特征)。代码保留默认关(volume_filter_enabled=False)。**至此震荡/环境/量能三类识别路线全部证伪, 退出端(峰值回撤止盈)仍是唯一有效结构。**

### 除权防御: manage.py split + 持仓除权监控 (2026-09-01)
**背景**: 审查发现实盘对除权(送股/转增/配股/分红)无任何处理——持仓 shares/avg_cost 永不调整。遇除权后果: ①持仓股票送转后 qfq 数据价已下调但股数未增 → 市值虚亏、卖出数量错误(只卖一半); ②qfq 历史漂移(除权后重下数据, 复权因子更新 → 历史价全变 → 信号历史漂移); ③ETF 用 Sina 不复权 → 除息日价格跳空触发假"价格回归"卖出信号。
**实现**:
①`manage.py split <symbol> <ratio>`: 送转调整——shares×ratio、avg_cost÷ratio、总成本不变; ratio 为送转后倍数(每10股送转20股→3, 与 akshare 送转总比例 20 对应); 校验 ratio∈(0,10]、持仓存在、股数取整; 操作前自动备份。现金分红不改变股数无需 split(提示说明)。
②`validate.check_ex_rights`: 每日校验新增持仓除权监控——akshare `stock_fhps_detail_em` 查询持仓标的, 除权除息日 ∈ [今天-3, 今天+7] 且方案=实施分配 → warn 预警(已除权→紧急提示 split; 未来→预告并给出送转倍数); 仅查持仓标的(9-10 只), 网络失败/接口变更静默跳过不阻断主流程。
**测试**: 新建 tests/test_manage.py 10 用例(split 正常/取整/越界/非持仓; 除权预警窗口/紧急/远期/接口异常/无持仓不查询), 全套 291 全绿。真实持仓 9 只验证: 当前无近期除权事件。
**已知边界**: 配股需手工 adjust; 现金分红未自动入账(市值微虚, 影响小); 除权日如遇策略执行仍依赖人工按预警操作。

### 实盘报告与流程多项修复 (2026-08-31 ~ 09-01)
**被跳过候选显示窗口 5→1 天 (2026-08-31)**: 实盘报告"待买入被跳过"原显示近 5 天候选, 跨周末残留旧数据(长川 8-27 案例); 改为仅当日——与 `_auto_fill_pool` 当日候选的执行语义严格对齐, 候选若仍有效当日重新触发会生成新的当日记录。新增 2 测试。
**被跳过列表排序 (2026-08-31)**: 最近日期在前, 同日期内按偏离度降序(原仅按偏离度)。
**月度收益分池 (2026-09-01)**: 柱状图三组柱(总体/股票/基金) + 热力表每年三行, 各池以各自初始资金为基准, 全年列独立复合。
**权益曲线逐日重估 (2026-09-01)**: 原曲线只在事件日有点——基金池最后事件 7-30 时 7 月末点缺 7-31 浮动重估, 月度"股票+基金"≠总体(差 936 = 无事件日持仓浮动); 改为交易日全集逐日重估, 各池月末点同日、池和=总体逐日成立(差值 ≤1 元); 今日点用 positions 口径替换防同日双点台阶。新增 1 测试。
**daily_signal 先执行后扫描 (2026-09-01)**: 原"扫描→执行→补仓"两步——昨日卖出成交释放的名额在扫描后才生效, 当日候选先被"已达最大持仓数"拦截再由 `_auto_fill_pool` 补入(大族 9-1 案例); 改为执行移至扫描前, 名额检查(实时读 positions)基于执行后真实持仓直接下单, 与 unlimited_signal 流程统一。资金时序正确(昨日卖出今日到账, 买入 T+2 开盘), 与 2026-08-26 名额预释放否决不冲突(该否决针对"今日卖出信号未成交即释放")。新增 1 测试。
**backtest.py docstring 修正 + 涨停近似量化 (2026-09-01)**: `_run_pool` 注释"手续费置零(对齐旧基线)"为过时——实际传真实费率(佣金万3/印花千1/过户万0.1/最低5元, 与实盘 calc_fee 一致)。涨停 A 方案(T 日 close 近似 vs 实盘 T+1 open 精确)影响量化: 全池回测(2020-2026, 49 标的)执行日 open 涨停买入仅 **2/1146 笔**, 合计 +¥29,547 ≈ **+4.0% 已实现盈亏**(高估上界, 实盘拒单后资金再配置会进一步缩小)——机制相对比较(基线 vs 变体)两侧处理一致不受影响, 维持"已知差异不修"(引擎级修复投入产出比低)。
**环境说明 (2026-09-01)**: daily_signal_readme/setup_env 默认环境统一为 `akquant_032`(0.3.20 生产), `akquant_test`(0.2.22)保留为回退环境。

### portfolio_backtest 对齐实盘语义 6 项修复 (2026-09-01)
**背景**: 审查发现组合回测旧脚本(portfolio_backtest.py, 独立执行, **不参与 kalman.md 机制结论**——结论全部来自 backtest.py 的 run_portfolio_backtest/AKQuant 引擎)与实盘 daily_signal 存在 6 处语义差异。**修复**:
①涨停/跌停基准价错误——原用执行日(T+1)当天收盘(未来信息, 拦截基本失效), 改为前交易日收盘(信号日, 对齐 orders.py prev_close 语义), is_kcb 三前缀 688/300/301;
②手续费缺失——复用 portfolio.calc_fee, 买入 avg_cost 含费、卖出 pnl 减费(对齐实盘口径);
③资金跨池共用——单现金池改为 stock/etf 分池独立(实盘"不跨池串用");
④名额口径——pool_pending 排除卖单与已持仓补仓单(对齐 daily_signal:294-295);
⑤补仓缺失——持仓标的 hold+target_pct>0 按实盘公式生成 refill 订单(不占名额), 成交加权合并持仓(原覆盖逻辑因信号路径限制从未触发), 同 symbol 双订单清理(sell 不同向替换);
⑥T+1 信号日过滤(防御)。
**结构**: 拆分 `_run_backtest_core`(无 I/O 可测) + 6 个纯函数(`_is_kcb/_pool_for/_prev_close_of/_limit_blocked/_merge_position/_refill_qty`); run_backtest 保留原 I/O 行为。
**测试**: 新建 tests/test_portfolio_backtest.py 21 用例(涨停拦截/费用恒等式/分池独立/名额/补仓合并/双订单清理/端到端恒等式), 全套 281 全绿。
**离线重跑对比**(catalog 2020-2026, 49 标的): 修复前 交易 3294/收益 +212.8%/夏普 1.50/回撤 -14.0% → 修复后 交易 3796(含补仓)/收益 +198.7%/夏普 1.13/回撤 -19.9%。收益 -14pp 来自: 手续费累计 ¥96,219 + 涨停拦截修正(高开追买被拒) + 分池限制; 修复后数字更贴近实盘真实表现(有费用、涨停保护生效、分池正确、补仓机制), 与旧 CSV 不可比。
...
### 两阶段下单三处修复 (2026-08-27)
**问题**: ①跨池名额串扰——`placed_new` 全局计数 + 股票/ETF 混合排序, 股票池下单挤占 ETF 池名额(8-27 股票 2 单使 ETF 阶段 2 仅下 1 单, 证券/半导体ETF 被挤到 `_auto_fill_pool` 补入); ②双重 cap——`_block` 存 capped_pct(0.19), `_auto_fill_pool` 再乘 max_pct → 0.038, 高价股(光智¥242)买不起 1 手被静默跳过、低价股(巨石)补仓量减半(200 股应为 500 股); ③`pool_pending_new` 重复计数——实时读 pending 使本轮新下的单也计入, 与 `placed_new` 重复 → 名额减半(股票池 3 名额只下 2 单)。**修复**: ①候选按池分组排序(`_sort_pool`: 池内补仓优先+偏离降序), `placed_new` 分池计数; ②`_deferred` 携带 `raw_target`(原始 target), `_block` 存原始值; ③名额检查用本轮开始前快照 `pre_pending`。**验证**: 修复后股票池 3/3 满(指南针/香农/光智按偏离)、ETF 池 3/3 满(科创板50/证券/半导体阶段 2 直接下单), 光智正常补入; 新增 4 个测试 + 测试污染防护(状态文件隔离 tmp, 8-27 曾污染生产 pending/execution_log), 全套 272 全绿。

### 回测超限买单排查 (2026-08-28)
**排查**: 组合回测发现股票池个别买单超单只上限(最大 ¥66,363 = 22.1% vs 上限 20%)。**结论**: 非策略/引擎 bug——策略 target 正确(¥18,000~¥57,000 全程无超限), 引擎取整正确(1100×信号价≤57,000)——根因是 **T+1 跳空缺口**: 按 T 日收盘价算股数, T+1 开盘价跳空(光智 2025-05-19 收 51.63 → 05-20 开 60.33, +16.9%)→ 成交金额超目标 16.4%。**回测与实盘行为一致**(daily_signal 同样 T 日算股数 T+1 成交, 高开时超计划金额), 属 T+1 执行机制固有风险, 无需修复; 组合回测结论不受影响(3 笔个案超限 0.3~10.6%)。**同时确认回测无跨池/双 cap/重复计数类问题**(分池独立跑 + 自维护计数 + 一次 cap)。
